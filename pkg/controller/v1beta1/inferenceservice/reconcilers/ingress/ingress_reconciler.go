...
func getServiceUrl(isvc *v1beta1.InferenceService, urlScheme string, isIngressEnabled bool) string {
	if isvc.Status.Components == nil {
		return ""
	}
...
func (ir *IngressReconciler) Reconcile(isvc *v1beta1.InferenceService) error {
	serviceHost := getServiceHost(isvc)
	serviceUrl := getServiceUrl(isvc, ir.ingressConfig.UrlScheme, ir.ingressConfig.Enabled)
	if serviceHost == "" || serviceUrl == "" {
		return nil
	}
	//Create ingress
	if ir.ingressConfig.Enabled {
